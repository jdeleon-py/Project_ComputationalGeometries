...
class Grid2:
	...
	def __init__(self) -> None:
		...
		self.grid = [[random.choice([0, 1]) for _ in range(0, Grid2.DIM)] for _ in range(0, Grid2.DIM)]
		self.cell_width = WINDOW_WIDTH / Grid2.DIM
		self.cell_height = WINDOW_HEIGHT / Grid2.DIM
		self.running = True

	def draw_grid(self) -> None:
		for row in range(0, Grid2.DIM):
			for column in range(0, Grid2.DIM):
				color = Grid2.BLACK
				if self.grid[row][column] == 1: color = Grid2.WHITE
